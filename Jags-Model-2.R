...
genMCMC = function( data , sName="s" , yName="y" ,  
                    numSavedSteps=10000 , saveName=NULL , thinSteps=1 ) { 
  require(rjags)
  #-----------------------------------------------------------------------------
  # THE DATA.
  # This function expects the data to be a data frame, 
  # with one component named y being a vector of integer 0,1 values,
  # and one component named s being a factor of subject identifiers.
  y = data[,yName]
  s = as.numeric(data[,sName]) # ensures consecutive integer levels
  
  # Checks that data is binary
  ...
  model {
    for ( i in 1:Ntotal ) {
      y[i] ~ dbern( theta[s[i]] ) #bernoulli distribution
    }
    for ( sIdx in 1:Nsubj ) {
      theta[sIdx] ~ dbeta( omega*(kappa-2)+1 , (1-omega)*(kappa-2)+1 )#Group mode and concentration
    }
    omega ~ dbeta( 8 , 2 ) # broad uniform #Set right skewed beta distribution for mode
    kappa <- kappaMinusTwo + 2
    kappaMinusTwo ~ dgamma( 4 , 0.1051249 )  #Gamma distribution with high mode and average sd
...
plotMCMC = function( codaSamples , data , sName="s" , yName="y" , 
  ...
  # Plot omega, kappa:
  openGraph(width=5*2,height=3.0)
  par( mfrow=c(1,2) )
  par( mar=c(3.5,3,2,2) , mgp=c(2.0,0.7,0) )
    ...
    for ( t1Idx in 1:Nidx ) {
      for ( t2Idx in 1:Nidx ) {
        ...
        if ( t1Idx > t2Idx) {  
          ...
          par( mar=c(3.5,3,1,1) , mgp=c(2.0,0.7,0) , pty="s" )
          nToPlot = 700
          ptIdx = round(seq(1,chainLength,length=nToPlot))
          plot ( mcmcMat[ptIdx,parName2] , mcmcMat[ptIdx,parName1] , cex.lab=1.0 ,
                 xlab=parName2 , ylab=parName1 , col="skyblue" )
          abline(0,1,lty="dotted")
        } else if ( t1Idx == t2Idx ) {
          par( mar=c(3.5,3,1,1) , mgp=c(2.0,0.7,0) , pty="m" )
          postInfo = plotPost( mcmcMat[,parName1] , cex.lab = 1.0 , 
                               compVal=compVal , ROPE=rope , cex.main=1.0 ,
                               xlab=parName1 , main="" )
          ...
          postInfo = plotPost( mcmcMat[,parName1]-mcmcMat[,parName2] , 
                            compVal=compValDiff , ROPE=ropeDiff , cex.main= 1.0 ,
